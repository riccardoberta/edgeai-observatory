...
Early TinyML work focused almost entirely on inference: train in the cloud, deploy a frozen model. On-device learning challenges that split by asking whether a deployed device can update its own model from locally collected data — for personalization, adaptation to drift, or privacy reasons (no raw data leaves the device). The main obstacle is memory: standard back-propagation needs to store activations, gradients, and optimizer state, which can be orders of magnitude larger than the SRAM available on a microcontroller. Lin et al. (2022) make a first concrete, hardware-validated case that training (not just inference) is possible within a 256 KB memory budget, via a combination of quantization-aware training tricks and selecting only a sparse subset of weights to update. TinyTrain (Kwon et al., ICML 2024) extends this line by tackling a second constraint that TinyTL/the 256 KB work left open — data scarcity at the edge — combining a task- and memory-aware sparse-update selection with a meta-learning pre-training phase so that on-device fine-tuning needs less labeled data, validated on real microcontroller and mobile hardware. A 2022 MDPI Electronics paper (Freitag) provides an earlier, real-hardware data point specifically for the training-plus-federated-learning combination: implementing the full training loop directly on microcontrollers for a keyword-spotting task and characterizing, through physical-device experiments rather than simulation, how federated-round frequency trades off against bandwidth usage and training time — a concrete answer to the physical-hardware-validation gap this concept's own open problems (and [[Federated_Learning]]'s) already name. A 2026-09-03 historical audit added a broader, highly-cited (360+) entry point missing from this concept: Saha, Sandha, and Srivastava's "Machine Learning for Microcontroller-Class Hardware: A Review" (IEEE Sensors Journal 2022) reviews the full MCU-class ML pipeline end-to-end and treats on-device training/adaptation as one first-class pipeline stage rather than an isolated topic — a general reference this concept's more narrowly-scoped anchors (Lin et al., Kwon et al.) benefit from being situated within.

## Key papers

[[2022_Saha_MLMicrocontrollerClassHardwareReview]] — broad, full-pipeline review of MCU-class ML (algorithm, compression, runtime, hardware) with on-device training/adaptation as one explicit pipeline stage; highly-cited general entry point for this concept.
...
[[2022_Freitag_OnDeviceTrainingMCUFederated]] — real-microcontroller-hardware implementation of on-device training combined with federated learning for keyword spotting, characterizing the round-frequency/bandwidth/training-time trade-off empirically rather than in simulation.
