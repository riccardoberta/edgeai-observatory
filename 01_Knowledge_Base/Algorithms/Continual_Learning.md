...
Neural networks trained sequentially on new tasks tend to overwrite what they learned before — catastrophic forgetting. A 2026-09-03 historical audit found that this concept, despite tracking EWC and iCaRL as its two dominant early paradigms, had never actually cited the paper that established the second major paradigm's specific mechanism: Lopez-Paz and Ranzato's Gradient Episodic Memory (GEM, NeurIPS 2017) reformulates continual learning as a constrained optimization problem, allowing new-task gradient updates only in directions that do not increase loss on a small stored sample of previous tasks' data — and, notably, demonstrates positive backward transfer (learning a new task can improve performance on earlier ones), an evaluation dimension now standard in the field. Kirkpatrick et al. (2017) introduce Elastic Weight Consolidation (EWC), which estimates the importance of each weight to previously learned tasks (via the Fisher Information Matrix) and penalizes large changes to important weights when learning something new, without needing to store old training data. EWC became a reference point for the whole continual/lifelong-learning subfield, though later work has shown its Fisher-based importance estimate can be inaccurate over long task sequences, motivating refinements. For EdgeAI specifically, continual learning is the natural counterpart to [[On-device_Learning]]: a device that adapts from local data over its deployed lifetime needs some mechanism to avoid forgetting earlier-learned behavior, especially since it usually cannot afford to store a full history of raw data. A more recent direction, motivated by sensor-data privacy, replaces gradient-based incremental updates with closed-form analytic ones: TS-ACL (Li et al., arXiv 2024) reformulates class-incremental updates for time-series classification as a recursive least-squares computation over a frozen embedding, avoiding both raw-data replay and the forgetting that iterative fine-tuning is prone to. A 2024 IEEE conference paper (Rüb et al.) tackles the same forgetting problem from a third angle, purpose-built for TinyML deployment: rather than protecting weight importance (EWC) or replaying raw/lightly-processed exemplars (iCaRL), it distills prior-task data into a compact synthetic dataset cheap enough to retain within microcontroller-class memory, paired with a model-size-adaptation mechanism that grows the network as new classes are added — directly addressing the memory-budget constraint that general-purpose continual-learning methods do not consider by design. A second 2021 TinyML-specific paper (Ravaglia et al., IEEE JETCAS) took a fourth angle predating Rüb et al.: quantized latent replay, storing compact low-bitwidth intermediate activations instead of raw exemplars, validated end-to-end on real embedded hardware (GAP8-class) within under 64 MB of memory — directly bridging this concept with [[Quantization]] and giving this concept its first hardware-validated (not simulated) TinyML continual-learning anchor.
...
[[2017_LopezPaz_GradientEpisodicMemory]] — GEM: constrained-optimization continual learning using a small stored exemplar sample, with positive backward transfer; near 5000 citations, one of the two foundational early paradigms alongside EWC.
...
[[2024_Rub_ContinualIncrementalTinyML]] — TinyML-specific incremental learning combining dataset distillation (compact synthetic memory) with model-size adaptation, targeting genuinely memory-constrained embedded devices; IEEE ICPS 2024.
